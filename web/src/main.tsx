...
async function initTelegramSDK() {
  try {
    if (await isTMA()) {
      init();
      initData.restore();
      console.debug('[tg] SDK initialized in Telegram WebView');
    } else {
      ...
      init();
      initData.restore();
      console.debug('[tg] mock Telegram environment for development');
    }
  } catch {
    console.debug('[tg] not in Telegram WebView, running in browser mode');
  }
}
